...
import { getProject, updateProjectBuildTimelineStyle } from "@/lib/store";
...
const Schema = z.object({
  ...
  style: z.object({
    ...
    descFontPx: z.number().int().min(9).max(22).optional(),
    ...
    descWeight: z.number().int().min(100).max(900).optional(),
    descItalic: z.boolean().optional(),
    descColor: z.string().regex(/^#[0-9a-fA-F]{6}$/).optional(),
    ...
    vksFontPx: z.number().int().min(9).max(20).optional(),
    vksColor: z.string().regex(/^#[0-9a-fA-F]{6}$/).optional(),
    vksWeight: z.number().int().min(100).max(900).optional(),
    vksItalic: z.boolean().optional(),
    translationFontPx: z.number().int().min(9).max(20).optional(),
    translationColor: z.string().regex(/^#[0-9a-fA-F]{6}$/).optional(),
    translationWeight: z.number().int().min(100).max(900).optional(),
    translationItalic: z.boolean().optional(),
    interpretationFontPx: z.number().int().min(9).max(20).optional(),
    interpretationColor: z.string().regex(/^#[0-9a-fA-F]{6}$/).optional(),
    interpretationWeight: z.number().int().min(100).max(900).optional(),
    interpretationItalic: z.boolean().optional(),
...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  updateProjectBuildTimelineStyle(projectId, user.id, parsed.data.buildId, parsed.data.style);
