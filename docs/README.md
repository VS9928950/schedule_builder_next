...
- В текущей версии также задокументированы:
  ...
  - новые поля исходного документа (`Тимлид`, `Фотографии от ответственного`, `Сопроводительные материалы`, `Баннер`);
  - новые вкладки `Анонсы` и `Отчет`;
  - импорт документа по ссылке Google Sheets (`docs.google.com/spreadsheets/d/...`) через серверное скачивание `.xlsx`.
